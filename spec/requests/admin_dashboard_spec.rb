...
describe 'AdminLogin' do
  ...
  describe 'dashboard numbers are scoped to the current tournament' do
    it 'should only show you the current tournaments figures' do
      ...
      columns(all('div.panel_contents tr')[1].text).should == ['Debate Teams', '1', '1', '1']
      columns(all('div.panel_contents tr')[2].text).should == ['Adjudicators', '2', '1', '0']
      columns(all('div.panel_contents tr')[3].text).should == ['Observers', '3', '1', '0']
    end
